...
- `docs/lecciones/<clase>.md` (RAIZ): memoria procedural, por CLASE de trabajo y
  ...
  `docs/lecciones/<clase>/referencias/<tema>.md` y dejes un puntero; lo
  mecanico lo hace `leccion partir <clase>` (informa; `--aplicar` mueve)—; la misma
...
- `docs/kimi-cli-uso-eficiente.md` (RAIZ): guia de uso eficiente de Kimi Code
  ...
  entre tareas).
- GitHub Copilot CLI (feature #86): no tiene archivos propios en el arnes. Lee
  `AGENTS.md` y los hooks de `.claude/settings.json` en carpetas confiadas
  (`trustedFolders` de `~/.copilot/config.json`); el Stop va en modo
  `claude-json` (JSON `decision: block`), que Claude Code tambien honra.
