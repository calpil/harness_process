...
- **GitHub Copilot CLI**: lee este `AGENTS.md` nativamente y los hooks de
  `.claude/settings.json` (formato Claude), pero solo en carpetas confiadas
  (`trustedFolders` de `~/.copilot/config.json`: abri `copilot` en la carpeta y
  acepta la confianza). El Stop bloquea con `decision: block`. Sin subagentes
  del arnes: aplica los roles como fases.
...
MODE="${1:-plain}"   # plain | gemini-json | codex-json | claude-json (feature #86)
...
elif [ "$MODE" = "claude-json" ]; then
    # Feature #86: el Stop de .claude/settings.json. Claude Code y Copilot CLI
    # (que lee ese mismo archivo, con la carpeta confiada) bloquean con el JSON
    # {"decision":"block","reason":...} por stdout y exit 0; el exit 2 con
    # stderr solo lo entiende Claude (medido con Copilot 1.0.83). Lo legible va
    # a stderr igual, y el reason lleva las ultimas lineas del check para que
    # el modelo vea QUE fallo. Los demas eventos siguen como plain: su stdout
    # es contexto para el agente.
    case "$EVENT" in
        stop|Stop|AfterAgent|SessionEnd|SessionStop)
            salida_stop="$(run_event 2>&1)" && rc_stop=0 || rc_stop=$?
            [ -n "$salida_stop" ] && printf '%s\n' "$salida_stop" >&2
            if [ "$rc_stop" -ne 0 ]; then
                detalle_stop="$(printf '%s\n' "$salida_stop" | tail -n 40 | tr -d '\r' \
                    | LC_ALL=C tr -d '\000-\010\013\014\016-\037' \
                    | sed -e 's/\\/\\\\/g' -e 's/"/\\"/g' -e 's/\t/\\t/g' \
                    | awk '{printf "%s\\n", $0}')"
                printf '{"decision":"block","reason":"Harness check fallo; corrige el estado del repo antes de cerrar (o documenta el bloqueo si no lo podes resolver). Detalle:\\n%s"}\n' "$detalle_stop"
            fi
            exit 0
            ;;
        *)
...
# Bootstrap for new required documentation files (e.g. when user updated only setup_harness.sh
# from a newer source but hasn't copied the new asset files yet). This prevents the
# "Falta el recurso requerido: UPDATING.md" error during incremental updates.
if [ ! -f "$ASSET_DIR/UPDATING.md" ]; then
    mkdir -p "$(dirname "$ASSET_DIR/UPDATING.md")"
    cat > "$ASSET_DIR/UPDATING.md" << 'UPDATING_BOOTSTRAP_EOF'
...
UPDATING_BOOTSTRAP_EOF
    echo "[Harness] UPDATING.md faltaba; se creó automáticamente (bootstrap para actualización)."
fi
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash \"$SURFACE_BASE/bin/harness-hook\" claude-json stop",
            "timeout": 120
          }
        ]
      }
