...
1. Lee `__HREL__roles/README.md`, `__HREL__feature_list.json`,
   `__HREL__progress/current.md` y `docs/constitution.md` (los principios que el
   spec y el plan deben cumplir).
2. **PEDI EL PAQUETE ANTES DE LEER NADA** (feature #56):
   `sh "__HREL__harness_cli" contexto --feature <id>` (o `--tema "<texto>"` si
...
4.0. PREGUNTALE al repo antes de leerlo entero:
   `sh "__HREL__harness_cli" buscar "<terminos del tema>"`. Devuelve archivo,
...
4.1. Revisa la memoria procedural del proyecto ANTES de disenar:
   `sh "__HREL__harness_cli" leccion list`. Si una leccion cubre la clase de
...
4.2. Si `docs/perfil-usuario.md` tiene entradas, respetalas: son como el usuario
   ...
   `sh "__HREL__harness_cli" perfil sugerir`, PROPONESELA al usuario y solo con
...
5.1. Con el spec completo, ejecuta el **ritual de aprobacion** (no lo saltees, no
   ...
   3. PREGUNTALE explicitamente si lo aprueba, junto con las decisiones
      pendientes que haya.
   4. Solo con su SI: `sh "__HREL__harness_cli" approve-spec --yes --nota "<como aprobo>"`.
...
6. Persiste el plan en `docs/plan-feature-<id>-<slug>.md` (en el `docs/` de la
   ...
   reviewer exige por AC). `__HREL__progress/current.md` queda como puntero vivo;
...
- **Declara la cuenta ANTES de delegar**:
  `sh "__HREL__harness_cli" revision --feature <id> --esperar-tareas <n>`.
...
- Anotalo APARTE en `__HREL__progress/`, con lo que viste y donde.
